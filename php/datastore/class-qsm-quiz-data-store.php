...
class QSM_Quiz_Data_Store implements QSM_Data_Store_Interface {
	...
	public function create( &$quiz ) {
		...
		if ( is_qsm_quizzes_migrated() ) {
			...
			if ( $quiz_insert ) {
				...
				$quiz->set_id( $quiz_id );
				$quiz_post_id = $this->save_quiz_post_type($quiz);
				/**
				 * Activate quiz theme
				 */
				 ...
				 * Store Quiz Meta
				 */
				$quiz_meta	 = array_merge( $quiz->get_settings(), $default_texts );
				if ( ! empty( $quiz_meta ) ) {
				 ...
				 */
				do_action('qsm_quiz_created', $quiz_id, $quiz);
			} else {
	 ...
	 * @return post ID
	 */
	public function save_quiz_post_type(&$quiz) {
		global $wpdb;
