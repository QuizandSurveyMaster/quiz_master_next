...
class QSMQuizApi {
	 ...
	 */
	function qsm_verify_api_key_settings( $api_key ) {
		$response = array(
			'success' => false,
			'message' => '',
		);
		...
		if ( $qsm_api_settings_serialized ) {
			...
			if ( ($api_key && "" != $api_key) && (isset($qsm_api_settings['api_key']) && "" != $qsm_api_settings['api_key'] && $api_key == $qsm_api_settings['api_key']) ) {
	...
	public function qsm_get_quiz_result_info( $request ) {
	...
	public function qsm_get_quiz_info( $request ) {
	...
	function qsm_api_quiz_submit( $request ) {
		...
		return $qmnQuizManager->submit_results( $options, $data );
	}
