...
class QMNPluginHelper
{
    ...
    {
        add_action('wp_ajax_qmn_question_type_change', [ $this, 'get_question_type_edit_content' ]);
        add_action('admin_init', [ $this, 'qsm_add_default_translations' ], 9999);
        add_action('qsm_saved_question', [ $this, 'qsm_add_question_translations' ], 10, 2);
        add_action('qsm_saved_text_message', [ $this, 'qsm_add_text_message_translations' ], 10, 3);
        add_action('qsm_saved_quiz_settings', [ $this, 'qsm_add_quiz_settings_translations' ], 10, 3);

        add_action('qsm_register_language_support', [ $this, 'qsm_register_language_support' ], 10, 3);
        add_filter('qsm_language_support', [ $this, 'qsm_language_support' ], 10, 3);
        add_action('wp_ajax_qsm_validate_result_submission', [ $this, 'qsm_validate_result_submission' ], 10, 3);
        add_action('wp_ajax_nopriv_qsm_validate_result_submission', [ $this, 'qsm_validate_result_submission' ], 10, 3);
    }
     ...
     */
    public function get_quizzes($include_deleted = false, $order_by = 'quiz_id', $order = 'DESC', $user_role = [], $user_id = '', $limit = '', $offset = '', $where = '')
    {
        global $wpdb;
     ...
     */
    public function generate_settings_section($section = 'quiz_options', $settings_fields = [])
    {
        global $mlwQuizMasterNext;
     ...
     * @return void
     */
    public function register_question_type($name, $display_function, $graded, $review_function = null, $edit_args = null, $save_edit_function = null, $slug = null, $options = [])
    {
        if (is_null($slug)) {
            $slug = strtolower(str_replace(' ', '-', $name));
        } else {
    ...
    public function display_question($slug, $question_id, $quiz_options)
    {
        ...
        if (is_serialized($question->answer_array) && is_array(maybe_unserialize($question->answer_array))) {
            ...
            $mlw_answer_array_correct                                  = [ 0, 0, 0, 0, 0, 0 ];
            $mlw_answer_array_correct[ $question->correct_answer - 1 ] = 1;
            $answers = [
                [ $question->answer_one, $question->answer_one_points, $mlw_answer_array_correct[0] ],
                [ $question->answer_two, $question->answer_two_points, $mlw_answer_array_correct[1] ],
                [ $question->answer_three, $question->answer_three_points, $mlw_answer_array_correct[2] ],
                [ $question->answer_four, $question->answer_four_points, $mlw_answer_array_correct[3] ],
                [ $question->answer_five, $question->answer_five_points, $mlw_answer_array_correct[4] ],
                [ $question->answer_six, $question->answer_six_points, $mlw_answer_array_correct[5] ],
            ];
        }
    ...
    public function get_questions_count($quiz_id = 0)
    {
        ...
        if (! empty($quiz_settings)) {
            ...
            $pages       = isset($settings['pages']) ? maybe_unserialize($settings['pages']) : [];
            if (! empty($pages)) {
    ...
    {
        $settings    = isset($question_data['question_settings']) ? maybe_unserialize($question_data['question_settings']) : [];
        ...
        $answers = isset($question_data['answer_array']) ? maybe_unserialize($question_data['answer_array']) : [];
        if (! empty($answers)) {
            ...
            foreach ($answers as $key => $ans) {
                if ('image' === $answerEditor) {
                    ...
                    $answer_text = isset($ans[0]) ? trim(htmlspecialchars_decode($ans[0], ENT_QUOTES)) : '';
        ...
        foreach ($this->question_types as $type) {
            if (strtolower(str_replace(' ', '-', $slug)) === $type['slug']) {
                if (! is_null($type['review'])) {
                    ...
                    $results_array = [ 'null_review' => true ];
                }
            }
        }
