...
class QSM_Fields {
	...
	public static function generate_section( $fields, $section ) {
			...
			foreach ( $fields as $field ) {
				...
				if ( ( isset( $_POST[ $field['id'] ] ) && 'multiple_fields' !== $field['type'] ) || 'selectinput' == $field['type'] ) {
					switch ( $field['type'] ) {
						...
						case 'number':
							$sanitized_value = intval( wp_unslash( $_POST[ $field['id'] ] ) );
							break;
		...
		<form action="" method="post">
			<?php
			wp_nonce_field( 'save_settings', 'save_settings_nonce' );
						...
						foreach ( $fields as  $field ) {
							...
							if ( isset( $field['option_tab'] ) && 'general' === $field['option_tab'] ) {
								if ( ! empty( $field['type'] ) && 'multiple_fields' === $field['type'] ) {
									self::generate_field( $field, $settings );
								} else {
									self::generate_field( $field, $settings[ $field['id'] ] );
								}
							}
						}
						?>
						...
						<select class="category_selection_random" multiple="" id="qsm-option-<?php echo esc_attr( $field['id'] ); ?>">
							...
							if ( $multiple_category_system ) {
								...
								foreach ( $cat_array as $single_cat ) {
									?>
									<option <?php echo in_array( $single_cat, $explode_cat, true ) ? 'selected' : ''; ?> value="<?php echo esc_attr( $single_cat ); ?>"><?php echo wp_kses_post( $single_cat ); ?></option>
									<?php
								}
							}
							?>
				...
				<fieldset class="buttonset buttonset-hide" data-hide='1'>
					...
					foreach ( $checkbox_field['options'] as $option ) {
						...
						if ( 'randomness_order' == $checkbox_field['id'] ) {
							...
							$is_checked = in_array( $option['value'], $selected_values, true );
						} else {
							...
							<select class="question_limit_category" name="category_select_key[]">
								...
								if ( ! empty( $categories['list'] ) ) {
									foreach ( $categories['list'] as $key => $single_cat ) {
										?>
										<option value="<?php echo esc_attr( $key ); ?>"><?php echo esc_attr( $single_cat ); ?></option>
										<?php
									}
								}
								?>
					...
					<div class = "select-category-question-limit-subdiv">
						<select class="question_limit_category" name="category_select_key[]">
							...
							if ( ! empty( $categories['list'] ) ) {
								foreach ( $categories['list'] as $key => $single_cat ) {
									?>
									<option <?php echo ( isset( $category_select_key [ $i ] ) && ( $key == $category_select_key[ $i ] ) ) ? 'selected' : ''; ?> value="<?php echo esc_attr( $key ); ?>"><?php echo esc_attr( $single_cat ); ?></option>
									<?php
								}
							}
							?>
