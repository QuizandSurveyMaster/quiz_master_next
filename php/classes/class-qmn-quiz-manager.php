...
class QMNQuizManager {
	...
	*/
	...
	public function qsm_get_question_quick_result() {
		...
		$correct_info_text = $mlwQuizMasterNext->pluginHelper->qsm_language_support( $correct_info_text, "correctanswerinfo" );
	...
	public function display_shortcode( $atts ) {
		...
		if ( isset( $_GET['result_id'] ) && '' !== $_GET['result_id'] ) {
			...
			$correct_answer_text = $mlwQuizMasterNext->pluginHelper->qsm_language_support( $correct_answer_text, "quiz-correct-answer-text-{$qmn_array_for_variables['quiz_id']}" );
			$wrong_answer_text = sanitize_text_field( $qmn_quiz_options->quick_result_wrong_answer_text );
			$wrong_answer_text = $mlwQuizMasterNext->pluginHelper->qsm_language_support( $wrong_answer_text, "quiz-wrong-answer-text-{$qmn_array_for_variables['quiz_id']}" );
			$quiz_processing_message = $mlwQuizMasterNext->pluginHelper->qsm_language_support( $qmn_quiz_options->quiz_processing_message, "quiz-processing-message-{$qmn_array_for_variables['quiz_id']}" );
			$qmn_json_data = array(
	...
	public function display_quiz( $options, $quiz_data, $question_amount, $shortcode_args = array() ) {
		...
		$qmn_json_data['error_messages'] = array(
			'email'     => $mlwQuizMasterNext->pluginHelper->qsm_language_support( $options->email_error_text, "quiz-email-error-text-{$options->quiz_id}" ),
			'number'    => $mlwQuizMasterNext->pluginHelper->qsm_language_support( $options->number_error_text, "quiz-number-error-text-{$options->quiz_id}" ),
			'incorrect' => $mlwQuizMasterNext->pluginHelper->qsm_language_support( $options->incorrect_error_text, "quiz-incorrect-error-text-{$options->quiz_id}" ),
			'empty'     => $mlwQuizMasterNext->pluginHelper->qsm_language_support( $options->empty_error_text, "quiz-empty-error-text-{$options->quiz_id}" ),
		);
	...
	public function display_pages( $options, $quiz_data ) {
		...
		if ( count( $pages ) > 1 && ( ! empty( $options->message_before ) || ( 0 == $options->contact_info_location && $contact_fields ) ) ) {
			...
			$message_before              = $mlwQuizMasterNext->pluginHelper->qsm_language_support( $message_before, "quiz-message-before-{$options->quiz_id}" );
		...
		if ( 1 == count( $pages ) ) {
			...
			foreach ( $pages[0] as $question_id ) {
				...
				if ( 0 == $question['comments'] ) {
					?>
					<input type="text" class="qsm-question-comment qsm-question-comment-small mlw_qmn_question_comment" id="mlwComment<?php echo esc_attr( $question_id ); ?>" name="mlwComment<?php echo esc_attr( $question_id ); ?>" placeholder="<?php echo esc_attr( $mlwQuizMasterNext->pluginHelper->qsm_language_support( $options->comment_field_text, "quiz-comment-text-{$options->quiz_id}" ) ); ?>" onclick="qmnClearField(this)" />
					<?php
				}
				if ( 2 == $question['comments'] ) {
					?>
					<textarea class="qsm-question-comment qsm-question-comment-large mlw_qmn_question_comment" id="mlwComment<?php echo esc_attr( $question_id ); ?>" name="mlwComment<?php echo esc_attr( $question_id ); ?>" placeholder="<?php echo esc_attr( $mlwQuizMasterNext->pluginHelper->qsm_language_support( $options->comment_field_text, "quiz-comment-text-{$options->quiz_id}" ) ); ?>" onclick="qmnClearField(this)" ></textarea>
					<?php
				}
				// Checks if a hint is entered.
				if ( ! empty( $question['hints'] ) ) {
					echo '<div class="qsm-hint qsm_hint mlw_qmn_hint_link qsm_tooltip">' . esc_html( $mlwQuizMasterNext->pluginHelper->qsm_language_support( $options->hint_text, "quiz-hint-text-{$options->quiz_id}" ) ) . '<span class="qsm_tooltiptext">' . wp_kses_post( $mlwQuizMasterNext->pluginHelper->qsm_language_support( $question['hints'], "hint-{$question_id}" ) ) . '</span></div>';
				}
				?>
					</div>
				<?php
			...
			if ( 0 == $options->comment_section ) {
				...
				$message_comments = $mlwQuizMasterNext->pluginHelper->qsm_language_support( $message_comments, "quiz-message-comments-{$options->quiz_id}" );
				?>
					<div class="quiz_section quiz_begin">
			...
			if ( ! empty( $options->message_end_template ) || ( 1 == $options->contact_info_location && $contact_fields ) ) {
				...
				$message_after = $mlwQuizMasterNext->pluginHelper->qsm_language_support( $message_after, "quiz-message-end-{$options->quiz_id}" );
				?>
		...
		add_action(
			...
			<script type="text/template" id="tmpl-qsm-pagination-<?php echo esc_attr( $options->quiz_id ); ?>">
				<div class="qsm-pagination qmn_pagination border margin-bottom">
					<a class="qsm-btn qsm-previous qmn_btn mlw_qmn_quiz_link mlw_previous" href="#"><?php echo esc_html( $mlwQuizMasterNext->pluginHelper->qsm_language_support( $options->previous_button_text, "quiz-previous-button-text-{$options->quiz_id}" ) ); ?></a>
					<span class="qmn_page_message"></span>
					<div class="qmn_page_counter_message"></div>
					<div class="qsm-progress-bar" style="display:none;"><div class="progressbar-text"></div></div>
					<a class="qsm-btn qsm-next qmn_btn mlw_qmn_quiz_link mlw_next" href="#"><?php echo esc_html( $mlwQuizMasterNext->pluginHelper->qsm_language_support( sanitize_text_field( $options->next_button_text ), "quiz-next-button-text-{$options->quiz_id}" ) ); ?></a>
					<input type='submit' class='qsm-btn qsm-submit-btn qmn_btn' value='<?php echo esc_attr( $mlwQuizMasterNext->pluginHelper->qsm_language_support( $options->submit_button_text, "quiz-submit-button-text-{$options->quiz_id}" ) ); ?>' />
				</div>
			</script>
				<?php
			}
	...
	public function display_begin_section( $qmn_quiz_options, $qmn_array_for_variables ) {
		...
		if ( ! empty( $qmn_quiz_options->message_before ) || ( 0 == $qmn_quiz_options->contact_info_location && $contact_fields ) ) {
			...
			<div class="qsm-auto-page-row quiz_section <?php echo esc_attr( $animation_effect ); ?> quiz_begin">
				...
				$message_before = $mlwQuizMasterNext->pluginHelper->qsm_language_support( $message_before, "quiz-message-before-{$qmn_quiz_options->quiz_id}" );
	...
	public function display_comment_section( $qmn_quiz_options, $qmn_array_for_variables ) {
		...
		if ( 0 == $qmn_quiz_options->comment_section ) {
			...
			$message_comments = $mlwQuizMasterNext->pluginHelper->qsm_language_support( $message_comments, "quiz-message-comments-{$qmn_quiz_options->quiz_id}" );
			...
			<div class="qsm-auto-page-row quiz_section quiz_end <?php echo esc_attr( $qsm_d_none ); ?>">
				...
				if ( ! empty( $qmn_quiz_options->message_end_template ) ) {
					...
					<?php
						...
						$message_end = $mlwQuizMasterNext->pluginHelper->qsm_language_support( $message_end, "quiz-message-end-{$qmn_quiz_options->quiz_id}" );
				...
				?>
				<input type='submit' class='qsm-btn qsm-submit-btn qmn_btn' value="<?php echo esc_attr( $mlwQuizMasterNext->pluginHelper->qsm_language_support( $qmn_quiz_options->submit_button_text, "quiz-submit-button-text-{$qmn_quiz_options->quiz_id}" ) ); ?>" />
			</div>
			<?php
		} else {
			?>
...
function qmn_require_login_check( $display, $qmn_quiz_options, $qmn_array_for_variables ) {
	...
	if ( 1 == $qmn_quiz_options->require_log_in && ! is_user_logged_in() ) {
		...
		$mlw_message = $mlwQuizMasterNext->pluginHelper->qsm_language_support( $mlw_message, "quiz-require-login-{$qmn_quiz_options->quiz_id}" );
...
function qmn_total_user_tries_check( $display, $qmn_quiz_options, $qmn_array_for_variables ) {
	...
	if ( 0 != $qmn_quiz_options->total_user_tries ) {
		...
		if ( $mlw_qmn_user_try_count >= $qmn_quiz_options->total_user_tries ) {
			...
			$mlw_message       = $mlwQuizMasterNext->pluginHelper->qsm_language_support( $mlw_message, "quiz-user-tries-too-many-times-{$qmn_quiz_options->quiz_id}" );
			$mlw_message       = apply_filters( 'mlw_qmn_template_variable_quiz_page', $mlw_message, $qmn_array_for_variables );
			$display          .= $mlw_message;
		}
	}
...
function qmn_total_tries_check( $display, $qmn_quiz_options, $qmn_array_for_variables ) {
	...
	if ( 0 != $qmn_quiz_options->limit_total_entries ) {
		...
		if ( $mlw_qmn_entries_count >= $qmn_quiz_options->limit_total_entries ) {
			...
			$mlw_message       = $mlwQuizMasterNext->pluginHelper->qsm_language_support( $mlw_message, "quiz-limit-total-entries-message-{$qmn_quiz_options->quiz_id}" );
			$mlw_message       = apply_filters( 'mlw_qmn_template_variable_quiz_page', $mlw_message, $qmn_array_for_variables );
			$display          .= $mlw_message;
			$qmn_allowed_visit = false;
		}
...
function qmn_pagination_check( $display, $qmn_quiz_options, $qmn_array_for_variables ) {
	if ( 0 != $qmn_quiz_options->pagination ) {
		...
		$qmn_json_data['pagination'] = array(
			...
			'total_questions'  => $total_questions,
			'previous_text'    => $mlwQuizMasterNext->pluginHelper->qsm_language_support( sanitize_text_field( $qmn_quiz_options->previous_button_text ), "quiz-previous-button-text-{$qmn_quiz_options->quiz_id}" ),
			'next_text'        => $mlwQuizMasterNext->pluginHelper->qsm_language_support( sanitize_text_field( $qmn_quiz_options->next_button_text ), "quiz-next-button-text-{$qmn_quiz_options->quiz_id}" ),
		);
	}
	return $display;
}
