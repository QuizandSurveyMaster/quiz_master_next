...
class QMNQuizManager {
	...
	*/
	...
	public function ajax_submit_results() {
		...
		if ( '1' === isset($qsm_option['quiz_options']['not_allow_after_expired_time']) && isset( $_POST['currentuserTime'] ) && sanitize_text_field( wp_unslash( $_POST['currentuserTime'] ) ) > $dtUtcDate && ! empty($dateStr) ) {
			echo wp_json_encode(
				array(
