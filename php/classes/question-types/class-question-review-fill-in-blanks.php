	...
	private function process_sequentially() {
		...
		if ( $total_user_answers <= $total_correct_answer ) {
			foreach ( $this->user_answer as $user_answer_key => $user_answer ) {
				...
				}elseif ( 1 !== intval($case_sensitive) && $this->prepare_for_string_matching( $user_answer ) === $this->prepare_for_string_matching( $this->correct_answer[ $user_answer_key ] ) ) {
					$user_correct_ans++;
					$this->points       += $this->answer_array[ $user_answer_key ][1];
				}
			}
			...
			foreach ( $this->correct_answer as $correct_answer_key => $correct_answer ) {
				...
				}elseif ( 1 !== intval($case_sensitive) && $this->prepare_for_string_matching( $correct_answer ) === $this->prepare_for_string_matching( $this->user_answer[ $correct_answer_key ] ) ) {
					$user_correct_ans++;
					$this->points       += $this->answer_array[ $correct_answer_key ][1];
				}
			}
