...
function qsm_ajax_save_pages() {
	if ( !isset( $_POST['nonce'] ) || ! wp_verify_nonce( $_POST['nonce'], 'ajax-nonce-sandy-page' ) ) {
		die( 'Busted!' );
	}
...
function qsm_send_data_sendy() {
	if ( !isset( $_POST['nonce'] ) || ! wp_verify_nonce( $_POST['nonce'], 'ajax-nonce-sendy-save' ) ) {
		die( 'Busted!' );
	}
...
function qsm_delete_question_question_bank() {
	if ( !isset( $_POST['nonce'] ) || ! wp_verify_nonce( $_REQUEST['nonce'], 'delete_question_question_bank_nonce' ) ) {
		echo wp_json_encode(
			array(
				'success' => false,
				'message' => __(
...
function qsm_delete_question_from_database() {
	if ( !isset( $_POST['nonce'] ) ||  ! wp_verify_nonce( $_REQUEST['nonce'], 'delete_question_from_database' ) ) {
		echo wp_json_encode(
			array(
				'success' => false,
				'message' => __(
	...
	$question_id = isset( $_POST['question_id'] ) ? intval( $_POST['question_id'] ) : '';
	if ( $question_id ) {
		global $wpdb;
