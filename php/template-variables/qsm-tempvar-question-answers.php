	...
	foreach ( $total_answers as $single_answer_key => $single_answer ) {
		...
		if ( 0 == $form_type && ( 0 === intval( $grading_system ) || 3 === intval( $grading_system ) ) ) {
			...
			if ( $is_user_answer && $is_answer_correct ) {
				$question_with_answer_text .= '<span class="qsm-text-correct-option qsm-text-user-correct-answer ' .  $class . ' ' . $image_class . '"' . qsm_answer_status_describedby( 'your-correct' ) . '>' . $show_user_answer .$caption.$close_span;
			} elseif ( ! $is_user_answer && $is_answer_correct && 1 != $hide_correct_answer ) {
				$question_with_answer_text .= '<span class="qsm-text-correct-option ' .  $class . ' ' . $image_class . '"' . qsm_answer_status_describedby( 'correct' ) . '>' . $show_user_answer .$caption.$close_span;
			} elseif ( $is_user_answer && ! $is_answer_correct ) {
				$question_with_answer_text .= '<span class="qsm-text-wrong-option ' .  $class . ' ' . $image_class . '"' . qsm_answer_status_describedby( 'your-incorrect' ) . '>' . $show_user_answer .$caption.$close_span;
			} else {
				...
				$question_with_answer_text .= '<span class="qsm-text-correct-option ' .  $class . ' ' . $image_class . '"' . qsm_answer_status_describedby( 'your-answer' ) . '>' . $show_user_answer .$caption.$close_span;
			} else {
				$question_with_answer_text .= '<span class="qsm-text-simple-option ' .  $class . ' ' . $image_class . '">' . $show_user_answer .$caption.$close_span;
		}
