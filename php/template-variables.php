...
add_filter( 'mlw_qmn_template_variable_results_page', 'mlw_qmn_variable_time', 10, 2 );
...
/**
 * Replaces the variable %CURRENT_TIME% and displays the current date
 *
 * @param  string $content The contents of the results page
 * @param  array  $results The array of all the results from user taking the quiz
 * @return string Returns the contents for the results page
 */
function mlw_qmn_variable_time( $content, $results ) {
	$date = date_i18n( get_option( 'time_format' ), wp_timezone_string() );
	$content = str_replace( '%CURRENT_TIME%', $date, $content );
	return $content;
}
