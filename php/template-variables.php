...
function qsm_questions_answers_shortcode_to_text( $mlw_quiz_array, $qmn_question_answer_template, $questions, $qmn_questions, $answer, $qsm_question_cnt, $total_question_cnt ) {
					...
					foreach ( $total_answers as $single_answer ) {
						...
						if ( 0 == $form_type && ( 0 == $quiz_system || 3 == $quiz_system ) ) {
							if ( isset( $single_answer[2] ) && 1 == $single_answer[2] ) {
								...
								if ( 0 == strcasecmp( $actual_answer, htmlspecialchars_decode( $current_answer_zero, ENT_QUOTES ) ) ) {
						...
						if ( isset( $answer['question_type'] ) && ( 4 == $answer['question_type'] || 10 == $answer['question_type'] ) ) {
							...
							foreach ( $total_answers as $single_answer ) {
								...
								if ( $new_array_user_answer ) {
									foreach ( $new_array_user_answer as $new_array_value ) {
										...
										if ( htmlspecialchars_decode( $current_answer_text, ENT_QUOTES ) == htmlspecialchars_decode( $new_array_value, ENT_QUOTES ) ) {
											$is_answer_correct = true;
											break;
										}
									}
							...
							if ( 13 == $answer['question_type'] ) {
								...
								foreach ( $total_answers as $single_answer ) {
									...
									$user_ans_decode_value = htmlspecialchars_decode( $answer_value, ENT_QUOTES );
									$single_ans_decode_value = htmlspecialchars_decode( $single_answer_option, ENT_QUOTES );
									
									if ( isset( $single_answer[2] ) && 1 == $single_answer[2] && $user_ans_decode_value == $single_ans_decode_value ) {
										$question_with_answer_text .= '<span class="qsm-text-correct-option qsm-text-user-correct-answer ' . $image_class . '">' . $show_user_answer . '</span>';
									} elseif ( isset( $single_answer[2] ) && 1 == $single_answer[2] ) {
										$question_with_answer_text .= '<span class="qsm-text-correct-option' . $image_class . '">' . htmlspecialchars_decode( $show_user_answer, ENT_QUOTES ) . '</span>';
	...
	if ( isset( $question_settings['answerEditor'] ) && 'image' === $question_settings['answerEditor'] && 'NA' !== $answer_2 ) {
...
function qsm_get_question_maximum_points( $question = array() ) {
	...
	if ( ! empty( $question ) && isset( $question['answers'] ) ) {
		...
		if ( 4 == $question['question_type_new'] || 10 == $question['question_type_new'] ) {
