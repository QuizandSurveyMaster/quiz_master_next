...
function qsm_get_quiz_result_info( WP_REST_Request $request ) {
	...
	$verification = $mlwQuizMasterNext->qsm_api->qsm_verify_api_key_settings($api_key_param);

	if ( $verification['success'] ) {
		$response = $mlwQuizMasterNext->qsm_api->qsm_get_quiz_result_info($request);
	} else {
		$response = array(
			'success' => false,
...
function qsm_get_quiz_info( WP_REST_Request $request ) {
	...
	if ( $verification['success'] ) {
		$response = $mlwQuizMasterNext->qsm_api->qsm_get_quiz_info($request);
	} else {
		$response = array(
			'success' => false,
			'message' => $verification['message'],
...
function qsm_api_quiz_submit( $request ) {
    ...
    if ( $qsm_api_settings_serialized ) {
        ...
        if ( isset($qsm_api_settings['allow_submit_quiz']) && "1" == $qsm_api_settings['allow_submit_quiz'] ) {
            global $mlwQuizMasterNext;
            return rest_ensure_response($mlwQuizMasterNext->qsm_api->qsm_api_quiz_submit($request));
        } else {
            $response = array(
                'display'       => __('Admin does not allow submission of quiz via API.', 'quiz-master-next'),
                'redirect'      => false,
                'result_status' => array(
                    'save_response' => false,
                ),
...
function qsm_get_quiz_questions( WP_REST_Request $request ) {
	...
	if ( $verification['success'] ) {
		$response = $mlwQuizMasterNext->qsm_api->qsm_get_quiz_questions($request);
	} else {
		$response = array(
			'success' => false,
			'message' => $verification['message'],
