...
function mlw_options_styling_tab_content()
{
	...
	if (isset($_POST["save_style_options"]) && $_POST["save_style_options"] == "confirmation")
	{
		...
		{
			$mlwQuizMasterNext->alertManager->newAlert(printf(__('There has been an error in this action. Please share this with the developer. Error Code: %s', 'quiz-master-next'), '0015'), 'error');
		}
	}
	...
	<div id="tabs-7" class="mlw_tab_content">
		...
		<hr />
		<h3><?php _e('Custom Theme CSS', 'quiz-master-next'); ?></h3>
		<p><?php _e('For detailed help and guidance along with a list of different classes used in this plugin, please visit the following link:', 'quiz-master-next'); ?></p>
